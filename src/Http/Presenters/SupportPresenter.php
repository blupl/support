...
use Blupl\Support\Model\Support as Eloquent;
...
class SupportPresenter extends Presenter
{
        ...
        return $table->extend(function (TableGrid $table) {
            $table->column('action')
                ...
                ->value(function ($row) {
                    $html = [
                        app('html')->link(
                            handles("orchestra::support/reporter/{$row->id}/edit"),
                    ...
                    $roles = [
                        (int) $this->config->get('orchestra/foundation::support.admin'),
                        (int) $this->config->get('orchestra/foundation::support.member'),
                    ];

                    if (! in_array((int) $row->id, $roles)) {
                        $html[] = app('html')->link(
                            handles("orchestra::support/profile/{$row->id}/delete", ['csrf' => true]),
    ...
    {
        return $this->form->of('Support.registration', function (FormGrid $form) use ($model) {
            $form->resource($this, 'blupl/support::registration', $model);

            $form->fieldset(function (Fieldset $fieldset) {
                $fieldset->control('input:select', 'accredit_category')
                    ->options(['name'=>'Support'], 'name')
                    ->label('Accreditation Category');
                $fieldset->control('input:text', 'name_Support')
